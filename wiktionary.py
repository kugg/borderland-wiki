...
def invertlangnames():
    ...
    for ISOKey in langnames.keys():
        for ISOKey2 in langnames[ISOKey].keys():
            ...
            for index in xrange(1, len(lowercaselangname)):
                # So first we create all the possibilities with one letter gone
                invertedlangnames.setdefault(lowercaselangname[:index] +
                                             lowercaselangname[index + 1:],
                                             ISOKey2)
                # Then we switch two consecutive letters
                invertedlangnames.setdefault(lowercaselangname[:index - 1] +
...
class WiktionaryPage:
    ...
    def parseWikiPage(self, content):
            ...
            if '|' not in line:
                ...
                if bracketspos != -1 and colonpos != -1 and \
                   bracketspos < colonpos:
                    ...
                    lang = linkparts[0]
                    linkto = linkparts[1]
                    if len(lang) in range(2, 4):
                        self.addLink(lang + ':' + linkto)
                    continue
            # store empty lines literally, this necessary for the blocks we
            ...
            if line.strip()[0] == '=' and line.rstrip()[-2] == '=' or \
               '{{-' in line and '-}}' in line:
                ...
                if header.contents == u'trans':
                    ...
                    if 'width=' in lower:
                        continue
                    if '<!--left column' in lower:
                        continue
                    if '<!--right column' in lower:
                        continue
        ...
        for contentblock in splitcontent:
            ...
            # Let's start by describing what to do with content found under the
            # POS header
                ...
                for line in contentblock['text']:
                    if line[:3] == "'''":
                        ...
                        for part in line.split(' '):
                            ...
                            if sample:
                                maybegender = part.replace("'", '').replace(
                                    "}", '').replace("{", '').lower()
                                if maybegender == 'm':
                                    gender = 'm'
                                if maybegender == 'f':
                    ...
                    if sample:
                        ...
                        if contentblock['header'].contents == 'noun':
                            theterm = Noun(lang=contentblock['context']['lang'],
                                           term=sample, gender=gender)
                        if contentblock['header'].contents == 'verb':
                            theterm = Verb(lang=contentblock['context']['lang'],
                                           term=sample)
                        sample = ''
                    if line[:1].isdigit():
                    ...
                    if line[:1] == "#":
                        ...
                        if definition:
                            ...
                            if contentblock['context']['lang'] not \
                               in self.entries:
                        ...
                        pos = line.find('<!--')
                        if pos < 4:
                            # A html comment at the beginning of the line means
                            # this entry already has disambiguation labels,
                            # great
                            pos2 = line.find('-->')
            ...
            if definition:
                ameaning = Meaning(term=theterm, definition=definition,
                                   label=label, examples=examples)
...
class Entry:
    ...
    On Wiktionaries that are still on first character capitalization, this means
    both [[Kind]] and [[kind]]. Terms in different languages can be described.
    Usually there is one entry for each language.
    ...
    def wikiWrap(self, wikilang):
        ...
        entry = wiktionaryformats[
            wikilang]['langheader'].replace(
                '%%langname%%',
                langnames[wikilang][self.entrylang]).replace(
        ...
        for pos in self.posorder:
            ...
            if meaning.hasSynonyms():
                entry += wiktionaryformats[wikilang]['synonymsheader'] + '\n'
                for meaning in meanings:
                    entry += '*' + meaning.getLabel() + "'''" + \
...
class Meaning:
    ...
    """
    def __init__(self, term, definition='', etymology='', synonyms=[],
    ...
    def wikiWrapSynonyms(self, wikilang):
        ...
        for synonym in self.synonyms:
            if not first:
                ...
                first = False
            wrappedsynonyms += synonym.wikiWrapForList(wikilang)
    ...
    def wikiWrapTranslations(self, wikilang, entrylang):
        ...
        language itself
        - This list of translations has to end up in a table with two
          columns
        - The first column of this table contains languages with names from
          A to M, the second contains N to Z
        - If a column in this list remains empty a html comment is put in
          that column
        ...
        if wikilang == entrylang:
            # When treating an entry of the same lang as the Wiktionary, we want
            # to output the translations in such a way that they end up sorted
            # alphabetically on the language name in the language of the current
            # Wiktionary
            alllanguages = self.translations.keys()
            alllanguages.sort(sortonname(langnames[wikilang]))
            wrappedtranslations = wiktionaryformats[
                wikilang]['transbefore'] + '\n'
            alreadydone = False
            for language in alllanguages:
                if language == wikilang:
                    continue  # don't output translation for the wikilang itself
            ...
            wrappedtranslations = wiktionaryformats[
                wikilang]['translang'].replace(
                    '%%langname%%',
                    langnames[wikilang][wikilang]).replace(
                        '%%ISOLangcode%%', wikilang) + ': '
            first = True
...
##        self.label=label
    ...
    def __getitem__(self):
        ...
        I don't know anymore why I added this, but I'm pretty sure it was
...
##
    def wikiWrapGender(self, wikilang):
        ...
        """
        if self.gender:
            return ' ' + wiktionaryformats[
                wikilang]['gender'].replace('%%gender%%', self.gender)
        else:
            return ''
    ...
    def wikiWrapAsExample(self, wikilang):
        ...
        """
        return wiktionaryformats[
            wikilang]['beforeexampleterm'] + self.term + \
            wiktionaryformats[wikilang]['afterexampleterm'] + \
            self.wikiWrapGender(wikilang)
    ...
    def wikiWrapForList(self, wikilang):
        ...
        """
        return '[[%s]]' % (self.term, self.wikiWrapGender(wikilang))
    ...
    def wikiWrapAsTranslation(self, wikilang):
        ...
        """
        return '[[%s]]%s' % (self.term, self.wikiWrapGender(wikilang))
...
class Header:
    ...
    def __init__(self, line):
        ...
        self.header = self.header.replace('{{', '').replace(
            '}}', '').strip().lower()
...
def temp():
    ...
    FemalePersonFromItalymeaning = Meaning(
        aword, definition=u'vrouwelijke persoon die uit [[Italië]] komt',
        label=u'NFemalePersonFromItaly',
        concisedef=u'vrouwelijke persoon uit Italië',
        examples=['Die vrouw is een Italiaanse'])
...
#    * [[Italiaan]]
    ...
    FemalePersonFromItalymeaning.addTranslations(detrans, entrans, frtrans,
                                                 ittrans)
    ...
    FromItalymeaning = Meaning(aword, definition=u'uit Italië afkomstig',
                               synonyms=[asynonym], label=u'AdjFromItaly',
                               concisedef=u'uit/van Italië',
                               examples=['De Italiaanse mode'])
    RelatedToItalianLanguagemeaning = Meaning(
        aword, definition=u'gerelateerd aan de Italiaanse taal',
        synonyms=[asynonym], label=u'AdjRelatedToItalianLanguage',
        concisedef=u'm.b.t. de Italiaanse taal',
        examples=['De Italiaanse werkwoorden', 'De Italiaanse vervoeging'])
    ...
    RelatedToItalianLanguagemeaning.addTranslations(detrans, detrans2,
                                                    detrans3, entrans)
    RelatedToItalianLanguagemeaning.addTranslations(frtrans2, frtrans,
                                                    ittrans, ittrans2)
...
def run():
    ...
    try:
        ...
        sys.exit("You do not have permission to edit %s"
                 % self.pagelink.sectionFreeTitle())
    if old != new:
        new = ea.repair(new)
        ea.showdiff(old, new)
        comment = ea.getcomment()
        try:
            ea.pagelink.put(new, comment=comment, minorEdit=False,
                            watchArticle=ea.options.watch,
                            anon=ea.options.anonymous)
