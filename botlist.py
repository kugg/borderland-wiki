...
def get(site=None):
    ...
    if site in cache:
        ...
        fn = pywikibot.config.datafilepath('botlists', 'botlist-%s-%s.dat'
        ...
        try:
            ...
            if file_age > 1 * 24 * 60 * 60:
                pywikibot.output(
                    u'Copy of bot user list is one day old, reloading')
                refresh(site)
        except OSError:
...
##    if new:
##        import config
##        pywikibot.output('Downloading All bot user lists for your accounts '
##                         'in user-config.py')
