...
def urlopen(url):
    req = urllib2.Request(
        url,
        headers={'User-agent': 'Pywikibot family generator 0.1 - pywikipediabot.sf.net'})
...
class NamespaceStorage(object):
    ...
    def addfromwiki(self, w):
        jdata = json.load(urlopen(
            w.api +
            "?action=query&format=json&meta=siteinfo&siprop=namespaces|namespacealiases"))
        ...
        if 'namespacealiases' in data:
            ...
            print("\n*** Notice: cannot retrieve namespace aliases for %s"
                  % w.lang)
    ...
    def add(self, ns, lang, translation):
        ...
        if ns == 0:  # never translate the article namespace
            return
        try:
            # self.f.namespace might return KeyError itself
            if translation != self.f.namespace(lang, ns):
                raise KeyError
        except KeyError:
    ...
    def output(self, indent):
        ...
        for nsid, langs in self.nsinfo.iteritems():
            ...
            for lang, translations in langs.iteritems():
                data += " " * indent
                data += ("self.namespaces[%(nsid)i][%(lang)r] = %(translations)r"
                         % locals())
                data += "\n"
        return data
...
class Wiki(object):
    ...
    def __init__(self, fromurl):
        ...
        try:
            self.version = bs.find(
                "meta",
                attrs={'name': "generator"})['content'].replace("MediaWiki ",
                                                                "")
        except Exception:
            self.version = "0.0"
