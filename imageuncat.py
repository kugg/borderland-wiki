...
# Ignore the templates in this really long list when looking for relevant
# categories
ignoreTemplates = [
    u'1000Bit',
    u'1922 cyc',
...
def uploadedYesterday(site=None):
    ...
    for item in site.logpages(number=5000, mode='upload',
                              start=yesterday.strftime(dateformat),
                              end=today.strftime(dateformat), newer=True,
                              dump=True):
...
def isUncat(page):
    ...
    for templateWithTrail in page.templates():
        ...
        if template in skipTemplates:
            ...
            pywikibot.output(u'Ignoring internationalization template ' +
                             template)
